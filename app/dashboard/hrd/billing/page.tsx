    ...
    return (
        <RoleGuard allowedRoles={['SUPER_ADMIN', 'MAINTAINER']}>
            <div className="space-y-6">
                <div>
                    <h1 className="text-[28px] font-bold font-display text-navy-900 leading-tight">Billing & Subscription</h1>
                    <p className="text-sm text-text-500 mt-1">Manage your organization's subscription plan and view invoices.</p>
                </div>

                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20">
                        <div className="w-10 h-10 border-4 border-navy-200 border-t-navy-600 rounded-full animate-spin mb-4" />
                        <p className="text-text-500 font-medium">Loading billing info...</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2">
                            <div className="card p-6 space-y-6">
                                <div className="flex items-start justify-between">
                                    <div>
                                        <h2 className="font-bold font-display text-navy-900 text-lg">Current Plan</h2>
                                        <p className="text-text-500 text-sm mt-1">Your active subscription details</p>
                                    </div>
                                    {sub ? (
                                        <span className="badge bg-success-bg text-success"><CheckCircle size={12} /> Active</span>
                                    ) : (
                                        <span className="badge bg-warning-bg text-warning"><AlertTriangle size={12} /> No Plan</span>
                                    )}
                                </div>
                                ...
                                {sub ? (
                                    <div className="bg-gradient-to-br from-navy-900 to-navy-700 rounded-xl p-6 text-white">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <p className="text-navy-200 text-sm font-medium">Plan</p>
                                    ...
                                    <div className="bg-surface-50 border border-surface-200 rounded-xl p-8 text-center">
                                        ...
                                        <p className="text-text-500">No active subscription. Contact sales to get started.</p>
                                    </div>
                                )}
                        ...
                        <div className="space-y-6">
                            <div className="card p-5">
                                <p className="text-text-500 text-xs font-semibold uppercase tracking-wider">Plan Status</p>
                                <p className="text-2xl font-black font-display text-navy-900 mt-2">{sub ? 'Active' : 'Inactive'}</p>
                            </div>
                            <div className="card p-5">
                                <p className="text-text-500 text-xs font-semibold uppercase tracking-wider">Invoices</p>
                                <p className="text-2xl font-black font-display text-navy-900 mt-2">{invoices.length}</p>
                            </div>
                            {sub && (
                                <div className="card p-5">
                                    <p className="text-text-500 text-xs font-semibold uppercase tracking-wider">Days Remaining</p>
                    ...
                    <div className="card overflow-hidden">
                        <div className="p-5 border-b border-surface-200 bg-surface-0">
                            <h2 className="font-bold font-display text-navy-900 flex items-center gap-2">
                                <FileText size={18} className="text-navy-600" /> Invoice History
                            </h2>
                        </div>
