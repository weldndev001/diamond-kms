        ...
        <div className="card overflow-hidden mt-6">
            <div className="p-5 border-b border-surface-200 bg-surface-0 flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
                <h2 className="font-bold font-display text-navy-900 flex items-center gap-2">
                    <Terminal size={18} className="text-navy-600" /> {t('logs.title')}
                </h2>
            ...
            <div className="divide-y divide-surface-100 max-h-[500px] overflow-y-auto">
                {loading && logs.length === 0 ? (
                    <div className="p-8 text-center text-text-400 text-sm">{t('logs.loading_logs')}</div>
                ) : logs.length === 0 ? (
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className="text-xs text-text-300 ml-auto whitespace-nowrap">
                                                {new Date(log.created_at).toLocaleString(t('common.language') === 'Indonesian' ? 'id-ID' : 'en-US')}
                                            </span>
                                        </div>
                                ...
                                {isExpanded && (
                                    <div className="pl-11 pr-4 pb-4 bg-surface-50/50">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 text-xs">
                                            ...
                                            <div>
                                                <span className="text-text-400 font-semibold block mb-1">{t('logs.log_id')}:</span>
                                        ...
                                        {log.stack_trace && (
                                            <div>
                                                <span className="text-text-400 text-xs font-semibold block mb-1">{t('logs.stack_trace')}:</span>
